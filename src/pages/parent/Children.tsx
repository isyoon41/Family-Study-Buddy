...
export default function Children() {
  ...
  const handleSaveTb = async (childId: string) => {
    ...
    setSaving(false);
    setBooks(b => ({ ...b, [childId]: [] })); // invalidate cache
  ...
  return (
    <div className="p-4 lg:p-6 max-w-3xl mx-auto space-y-5">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold text-gray-800">자녀 관리</h1>
          <p className="text-gray-400 text-sm">자녀 정보와 교재를 관리해요</p>
        </div>
        {!isDemo && (
          <button onClick={openAdd} className="bg-blue-500 text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-blue-600 transition">
            + 자녀 추가
          </button>
        )}
      </div>
      ...
      {children.length === 0 && !isDemo && (
        <div className="bg-white rounded-2xl p-10 text-center shadow-sm">
          ...
          <p className="text-gray-400">자녀를 추가해주세요</p>
          <button onClick={openAdd} className="mt-4 bg-blue-500 text-white px-6 py-2.5 rounded-xl font-bold text-sm hover:bg-blue-600">
            + 첫 자녀 추가
          </button>
        </div>
      )}
        ...
        return (
          <div key={child.id} className="bg-white rounded-2xl shadow-sm border border-gray-100">
            ...
            <div className="p-4 flex items-center gap-3">
              <div className="w-12 h-12 bg-blue-50 rounded-2xl flex items-center justify-center text-3xl">{child.avatar}</div>
              <div className="flex-1">
                <p className="font-bold text-gray-800">{child.name}</p>
                <p className="text-gray-400 text-sm">{child.grade} · PIN: {'●'.repeat(child.pin.length)}</p>
              </div>
              <div className="flex gap-2">
                {!isDemo && (
                  <>
                    <button onClick={() => openEdit(child)} className="text-xs text-gray-400 border rounded-lg px-3 py-1.5 hover:bg-gray-50">수정</button>
                    <button onClick={() => handleDelete(child.id)} className="text-xs text-red-400 border border-red-200 rounded-lg px-3 py-1.5 hover:bg-red-50">삭제</button>
                  </>
                )}
            ...
            {/* 교재 목록 */}
            {isExpanded && (
              <div className="border-t bg-gray-50 rounded-b-2xl p-4 space-y-3">
                <div className="flex justify-between items-center">
                  <p className="text-sm font-bold text-gray-600">📚 교재 진도 관리</p>
                  {!isDemo && (
                    <button onClick={() => { setShowAddTb(child.id); setTbForm({ subject:'',name:'',total_pages:0 }); }}
                      className="text-xs text-blue-500 font-medium hover:underline">+ 교재 추가</button>
                  )}
                </div>

                {childBooks.length === 0 && (
                  <p className="text-gray-400 text-xs text-center py-3">등록된 교재가 없어요</p>
                )}
                ...
                {childBooks.map(tb => {
                  ...
                  return (
                    <div key={tb.id} className="bg-white rounded-xl p-3 border border-gray-100">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <span className="text-xs bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full">{tb.subject}</span>
                          <p className="font-medium text-gray-800 text-sm mt-1">{tb.name}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-xs text-gray-400">{reached} / {tb.total_pages}p</p>
                ...
                {showAddTb === child.id && !isDemo && (
                  <div className="bg-blue-50 rounded-xl p-3 space-y-2 border border-blue-100 animate-fade-in">
                    <p className="text-sm font-bold text-blue-700">교재 추가</p>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => setShowAddTb(null)} className="px-4 border rounded-lg text-sm hover:bg-gray-50">취소</button>
                    </div>
                  </div>
                )}
              </div>
      ...
      {showAddChild && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl animate-fade-in">
            <h2 className="text-lg font-bold text-gray-800 mb-4">{editingChild ? '자녀 수정' : '자녀 추가'}</h2>
            <div className="space-y-3">
              <div>
                <label className="text-xs text-gray-500 mb-1 block">아바타</label>
                <div className="grid grid-cols-6 gap-1.5">
                  {AVATARS.map(a => (
                    <button key={a} onClick={() => setForm(f => ({ ...f, avatar: a }))}
                      className={`text-2xl p-1.5 rounded-xl transition ${form.avatar === a ? 'bg-blue-100 ring-2 ring-blue-400' : 'hover:bg-gray-100'}`}>
                      {a}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button onClick={() => setShowAddChild(false)}
                  className="px-5 border rounded-xl text-sm hover:bg-gray-50">취소</button>
              </div>
            </div>
          </div>
        </div>
